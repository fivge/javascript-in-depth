...
var isValid = function (code) {
  let stack = []; // new Array(code.length).fill("")
  let isEnd = false;

  for (let i = 0, len = code.length; i < len; i++) {
    if (code[i] === "<") {
      isEnd = false;
      stack[stack.length] = (stack[stack.length] || "") + code[i];
      continue;
    }

    if (code[i] === ">") {
      isEnd = true;
      stack[stack.length - 1] = (stack[stack.length - 1] || "") + code[i];
      continue;
    }
...
// let code = "<DIV> closed tags with invalid tag name<  <b>123</b> </DIV>";
let code = "<DIV> closed tags with invalid tag na>me  <b>123</b> </DIV>";

isValid(code);

// code.indexOf("")
// let regTag = /<(?<HtmlTag>[\w]+)[^>]*?>((?<Nested><\k<HtmlTag>[^>]*>)|</\k<HtmlTag>>(?<-Nested>)|.*?)*</\k<HtmlTag>>/
